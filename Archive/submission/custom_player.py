...
class RaisedPlayer(BasePokerPlayer):
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        if (
            ...
            and opp_profile not in ("aggressive", "calling_station", "maniac")
        ...
        if hand_strength < 0.40 and not self._has_draw_potential(hole_card, round_state):
            if call_amount > 0 and opp_profile != "maniac":
                return "fold"
            elif "call" in legal_actions:
                return "call"  # Checking for free (or maniac: let minimax decide)
        # ==========================================================
    ...
    def _opponent_response_value(self, child_values, state):
        ...
        dist = {
            "maniac":          {"raise": 0.97, "call": 0.02, "fold": 0.01},
    ...
    def _classify_opponent_profile(self, opp_stats):
        ...
        if opp_stats["total"] < 4:
            return "balanced"
        if opp_stats["raise_rate"] >= 0.80:
            return "maniac"   # always-raise bot; their raise carries zero information
